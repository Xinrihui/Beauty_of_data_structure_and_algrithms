...
class Solution:
    ...
    def solve(self, N, T):
        ...
        if sum(self.S) < self.T:  # 特殊处理：A 和 B 都不可能 达到 T, 两个人其实都输了
            return False  # 先手(A) 输
        ...
        for c in self.S:
            win = win or self.__process(c, t + c, visit | set([c]), state | (1 << (c - 1)))

        return win

    def __process(self, current, t, visit, state):

        if self.dp[state] != None:
            return self.dp[state]

        if t >= self.T:
            # print('win! current:{},t:{},visit:{}'.format(current, t, visit))

            return True

        # print('current:{},t:{},visit:{}'.format(current,t,visit))

        nextWin = False
        for c in self.S - visit:
            # print('c:{}'.format(c))

            nextWin = nextWin or self.__process(c, t + c, visit | set([c]), state | (1 << (c - 1)))
            # 若 nextWin 为 ture 则 后面的 表达式 不再计算
